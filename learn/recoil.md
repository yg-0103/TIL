...
1. Atom - atom은 리코일이 제공하는 전역상태의 작은 조각이다, 컴포넌트는 해당 상태를 구독하고 변화에 접근할 수 있다.
2. Selector - selector는 변형된 상태다, 동기 또는 비동기가 될 수 있고 atom과 마찬가지로 컴포넌트는 해당 상태를 구독하고 변화에 접근할 수 있다.
...
많은 다른 상태 매니지먼트 라이브러리가 있지만 왜 리코일일까?

1. 리코일은 리덕스같은 전역상태를 제공한다. 우리는 프랍으로써 상태를 공유하기위해 자식컴포넌트에 넘겨줄 필요가 없다. (props drilling 이 되지 않는다.)
2. 컴포넌트는 atom 혹은 selector를 사용해 상태를 구독하고 있으면 어디서든지 상태번화를 바로 반영할 수 있다
3. 리코일은 미니멀한 보일러플레이트 코드를 요구한다. 리덕스는 여전히 많은 보일러 플레이트 코드로 개발자들을 괴롭힌다.
